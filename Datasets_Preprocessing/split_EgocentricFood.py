...
def main():
    ...
    path = path_datasets+'/EgocentricFood';
    ...
    path_recognition = path_datasets+'/EgocentricFood_recognition'
        ...
        with open(imgs_file_list, 'r') as list_:
            for i,line in enumerate(list_):
                if(i > 0): # skip header
                    ...
                    #### Format variation: empty space at the end of the line
                    line = line[1:]
                    #line = line[1:-1]
                    for j,l in enumerate(line):
                        line[j] = float(l)
                    ##### Format variation: possibly more than one BB per line
                    categories_bbox_info[id_pos].append(line)
                    #print [im_num]+line
                    #for j in range(0,len(line), 4):
                    #    try:
                    #        categories_images[id_pos].append(im_num)
                    #        categories_bbox_info[id_pos].append(line[j:j+4])
                    #        print [im_num]+line[j:j+4]
                    #    except:
        ...
        for img in all_train_list:
            ...
            for occ in occurrences:
                ...
                try:
                    ...
                    print occ
                    print "image shape: "+ str(img_mat.shape)
                try:
...
def addNoFood():
    ...
    samples_proportion = [455, 196, 302] # number of train, val and test samples chosen (corresponds to mean samples per classes)
    #################################################
